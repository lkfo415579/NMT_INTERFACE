...
$(document).ready(function() {
	...
	$(document).click(function(e) {
		...
		if ((v_class == "tokenized_token hover") || (v_class == "xx") || (v_class == "spanClass")) {} else {
			$(".zz").hide();
		}
	});
	//多选项新添加end
	if (document.getElementById('news_title').addEventListener) {
		...
		document.getElementById('news_title').addEventListener('input', reset_idle, false);
	} else {
		document.getElementById('news_title').attachEvent('onpropertychange', function() {
...
/*
		...
		if (!$('input[name="from"]').is(':checked') || !$('input[name="to"]').is(':checked')) return false;
...
function show(data) {
	try {
		...
		$("#trans_target").empty();
		var nmt_select_menu = '請選擇對齊圖片: <select id="nmt-select-menu" onchange="onNmtSelectMenuChange()">';
			...
			if (_arr != "") {
				...
				if (v.sign == 1) {
					...
					if (v.errorCode != 0) {
						...
						// nmt-align
						var nmtAlign = v['translation'][0]['translated'][0]['alignment-raw'][0];
						var nmtSource = nmtAlign['source_align'].replace("\\\\", "\\").replace("\\\"", "\"");
						var nmtTarget = nmtAlign['target_align'].replace("\\\\", "\\").replace("\\\"", "\"");

						nmt_select_menu += '<option value="' +d+ '">' 
											+nmtSource.replace(new RegExp("@@ ", 'g'), '')+ 
											'</option>';

						var nmtSourceSplit = nmtSource.split(' ');
						var nmtTargetSplit = nmtTarget.split(' ');
						var nmtAlignInfo = JSON.parse(nmtAlign['alignInfo']).toString()
							.replace(new RegExp(",,,,,", 'g'), ",symcomma,symcomma,")
							.replace(new RegExp(",,,", 'g'), ",symcomma,").split(',');
						var sourceText = '<div id="nmt-align-table'+d+'-div" style="display: none"><table id="nmt-align-table'+d+'">';
						if (d == 0) {sourceText = sourceText.replace('none', 'block');}
						sourceText += "<tr><td>&nbsp;</td>";
						for (var i = 0; i < nmtSourceSplit.length; i ++){sourceText += '<td style="width:20px">' + nmtSourceSplit[i].replace("@@","@") + "</td>";}
						sourceText += "</tr>";

						for (var j = 0; j < nmtTargetSplit.length; j ++){
							var nmtAlignTableTr = "<tr><td>" + nmtTargetSplit[j] + "</td>";
							for (var i = 0; i < nmtSourceSplit.length; i ++){
								var colorValue = 255-Math.floor(nmtAlignInfo[j*(nmtSourceSplit.length+1)*5 + i*5 + 2] * 255)
								var sourceWord = nmtAlignInfo[j*(nmtSourceSplit.length+1)*5 + i*5 + 1]
								var targetWord = nmtAlignInfo[j*(nmtSourceSplit.length+1)*5 + i*5 + 0]
								nmtAlignTableTr += '<td title="'+sourceWord+ "/" +targetWord+'" style="background-color:rgb('+ colorValue + "," + colorValue + "," + colorValue +'); width:20px">&nbsp;</td>' ;
							}
							nmtAlignTableTr += "</tr>"
							sourceText += nmtAlignTableTr;
						}
						sourceText += "</table>";
						sourceText += "<h1>" + nmtSource+"</h1>";
                        sourceText += "<h1>" + nmtTarget+"</h1> </div>"
						$("#nmt-align").append(sourceText);

						// hover highlight
						$('#nmt-align-table'+d+' td').on('mouseover mouseout', function(){
							$(this).prevAll().addBack()
							.add($(this).parent().prevAll()
							.children(':nth-child(' + ($(this).index() + 1) + ')'))
							.toggleClass($(this).closest('table').attr('id'));
						});
						// end of nmt-align
						
						//sentences.forEach(showSentenceAlignment);
/*ben-command smt align in this block
...
*/
		...
		} //end for (var d=0;d<data_arr.length;d++ )
		$('#nmt-select').append(nmt_select_menu + '</select>')
	} //end try
	catch (exc) {
		alert(exc); /*for(key in data){alert(key);}*/
	}
}
			...
			if (j == 1) {
				...
				if ($('input[name="from"]:checked').val() == 'zh' || source == 'zh') {
					...
					if (token == temp_token) { //这是判断
						html += '<span class="tokenized_token" id="sp' + (i + 10000 * k) + 'txt" tokenorder="' + (i + 10000 * k) + '" >' + token + '</span>';
					} else {
						html += '<span class="tokenized_token" id="sp' + (x + 10000 * k) + 'txt" tokenorder="' + (x + 10000 * k) + '" >' + temp_token + '</span>';
					}
				} else {
					if (token == temp_token) {
						html += '<span class="tokenized_token" id="sp' + (i + 10000 * k) + 'txt" tokenorder="' + (i + 10000 * k) + '" >' + token + ' </span>';
				...
				if ($('input[name="to"]:checked').val() == 'zh') {
					...
					token = token.replace(/[ ]/g, "");
					html += '<span class="tokenized_token"  onclick="save(sp' + (i + 10000 * k) + ');" id="sp' + (i + 10000 * k) + '"  tokenorder="' + (i + 10000 * k) + '">' + token + '</span>';
					html += '<div class="zz" id="sp' + (i + 10000 * k) + 'an" style="background:#fff;width:200px;display:none;position:absolute;padding-top:0;z-index:999;" ></div>';
					/*var arr=[1,2,3,4];
					//alert(num3.length);
					for(var num2=0;num2<arr.length;num2++){
							//alert(num2)
							html+='<a style="display:block;" class="spanClass" onclick="change(sp' + (i + 10000*k) + ',\''+arr[num2]+'\')">'+arr[num2]+'</a>';
					}
					html+='<input type="txt" class="xx" value="'+token+'" id="input' + (i + 10000*k) + '" style="font-size:14px;width:140px;"><input type="button" value="使用"  onclick="inputChange(sp' + (i + 10000*k) + ',input'+(i + 10000*k)+')" style="width:50px; margin-left:5px;font-size:14px;"></div>';*/
				} else {
					//html += '<span class="tokenized_token" onclick="save(this);" tokenorder="' + (i + 10000*k) + '">' + token + ' </span>';
					html += '<span class="tokenized_token"  onclick="save(sp' + (i + 10000 * k) + ');" id="sp' + (i + 10000 * k) + '" tokenorder="' + (i + 10000 * k) + '">' + token + ' </span>';
					html += '<div class="zz" id="sp' + (i + 10000 * k) + 'an" style="background:#fff;width:200px;display:none;position:absolute;padding-top:0;z-index:999;" ></div>';
					...
					html+='<input type="txt" class="xx" value="'+token+'" style="font-size:14px;width:140px;" id="input' + (i + 10000*k) + '"><input type="button" value="使用"   onclick="inputChange(sp' + (i + 10000*k) + ',input'+(i + 10000*k)+')" style="width:50px; margin-left:5px;font-size:14px;"></div>';*/
				}
			}
		}
		html += '</div>';
...
function inputChange(obj, inputid) {
	var tdObj = $(obj);
	//var inputContent = $(inputid).val();
	var num = obj.id;
	var inputContent = inputid.id;
	var val = $("#" + inputContent).val();
	$("#" + num + "an").hide();
	/*var plist =obj.parentNode.getElementsByTagName("span");
	...
	var Tgt_Sen = tdObj.parent().find("span").text(); //获取目标语言的句子
...
function save(obj) {
	...
	var divlist = obj.parentNode.getElementsByTagName("div");
	var num = obj.id;
	var key = num.substr(2);
	var html = '';
	var divstyle = obj.id + "an";
	var left = document.getElementById(num).offsetLeft;
	var val = left - 70 + "px";
	document.getElementById(divstyle).style.marginLeft = val;
	var txtnum = obj.id + "txt";
	var txt = $("#" + num).html();
	var content = $("#" + txtnum).html();
	var _content = trim(content);
	var reg = /[\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\_|\+|\=|\||\\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g;
	//var reg=/[\,|\.|，|。]/;
	var sign = reg.test(content);
	for (var i = 0; i < plist.length; i++) {
		if (obj == plist[i]) {
			//$("#sp"+i+"an").toggle();
	...
	if (sign == true) {
		html += '<a style="display:block;cursor:pointer;" class="spanClass" onclick="change(' + num + ',\'' + content + '\')">' + content + '</a>';
		html += '<input type="txt" class="xx" value="' + txt + '" style="font-size:14px;width:140px;" id="input' + key + '"><input type="button" value="使用"   onclick="inputChange(' + num + ',input' + key + ')" style="width:50px; margin-left:5px;font-size:14px;"></div>';
		document.getElementById(divstyle).style.border = "1px solid #333";
		$("#" + divstyle).html(html);
	} else {
		var targetLang = $('input[name="to"]:checked').val();
		var query = {
		...
		$.post("translate.php",
			...
			function(data) {
				...
				if (errorCode != "0") {
					...
					for (var j = 0; j < senobj.length; j++) {
						arr[j] = v['translation'][j]['translated'][0]['tgt-tokenized'];
					}
					for (var num2 = 0; num2 < arr.length; num2++) {
						if (targetLang == "zh") {
							var aval = arr[num2].replace(" ", "");
							html += '<a style="display:block;cursor:pointer;" class="spanClass" onclick="change(' + num + ',\'' + aval + '\')">' + arr[num2] + '</a>';
						} else {
							html += '<a style="display:block;cursor:pointer;" class="spanClass" onclick="change(' + num + ',\'' + arr[num2] + '\')">' + arr[num2] + '</a>';
						}
					}
					html += '<input type="txt" class="xx" value="' + txt + '" style="font-size:14px;width:140px;" id="input' + key + '"><input type="button" value="使用"   onclick="inputChange(' + num + ',input' + key + ')" style="width:50px; margin-left:5px;font-size:14px;"></div>';
					document.getElementById(divstyle).style.border = "1px solid #333";
...
/*Buffer time*/
//add by ben, for translation delay buffer time
var isRun = 0;
var isLog = 0;
var last_content = '';
var last_target = '';
...
var idle_time = 0;
//every 100 ms
var update_idle_interval = setInterval(function() {
	if (idle_time < 55) {
		idle_time += 1;
	}
	if (isRun == 0 && idle_time > 3) {
		fanyi_change();
		isRun = 1;
	}
	if (isLog == 0 && idle_time > 20 && isRun == 1) {
		fanyi_log();
		isLog = 1;
	}
}, 100);
//every input change, reset
function reset_idle() {
	isRun = 0;
	isLog = 0;
	idle_time = 0;
...
//	alert('lang='+lang+'|targetLang='+targetLang);

	if (lang == targetLang || (lang == 'en' && targetLang == 'pt') || (lang == 'pt' && targetLang == 'en')){
		var src = $('input[name="from"]:checked').val();
		var src_text = $("#radioset").find('label[for="from_' + src + '"]').find(".ui-button-text").text();
		var trg = $('input[name="to"]:checked').val();
		var trg_text = $("#radioset2").find('label[for="to_' + trg + '"]').find(".ui-button-text").text();
		trans_msg = "<span>很抱歉！</span>" +
			"<span>暂不支持:</span>" +
			"<span class='warm-msg-text'>&nbsp;" + src_text + "&nbsp;</span>" +
			"<span>至</span>" +
			"<span class='warm-msg-text'>&nbsp;" + trg_text + "&nbsp;</span>" +
...
//log the translation input and result into db
function fanyi_log() {
	var _content = $('.animated').val();
	var _target = $('div#trans_target').text();
	if (_content == '' || _target == '' || (last_content == _content && last_target == _target)) {
		return false;
	}
	last_content = _content;
	last_target = _target;
	//	  var _target = $('#trans_target').val();
	var lang_from = $('input[name="from"]:checked').val();
	var lang_to = $('input[name="to"]:checked').val();
//	$("span#ben-input").html(_content);
//	$("span#ben-output").html(_target);
//	$("span#ben-json").html("From: "+lang_from + "|To: "+lang_to + "|Json: "+last_json);

/*
	if (lang_from == "auto") {
		var last_json_array = last_target.split("|||");
		var last_json = "";
		if (last_json_array.length > 0){
			last_json = last_json_array[0];
		}
		if (last_json != ""){
			//alert(last_json);
			var v = $.parseJSON(last_json);
			lang_from = v.sourceLang; 
		}
	}
*/
	...
	var data_package = {
		...
		"lang_from": lang_from,
		"lang_to": lang_to,
		"trans_msg": $(".trans-msg").val()
	};

	$.ajax({url:"bendev/translate_log.php",
			data:data_package,
			async:true,
			type:"post",
...
function onNmtSelectMenuChange(){
	var d = $('#nmt-select-menu').val();
	$("div[id^='nmt-align-table").css('display', 'none');
	$('#nmt-align-table'+d+'-div').css('display', 'block');
}
