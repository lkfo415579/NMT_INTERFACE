						...
						if (j==1) {
							...
							if($('input[name="to"]:checked').val() == 'zh'){
								...
								//html += '<span class="tokenized_token" onclick="save(this);" tokenorder="' + (i + 10000*k) + '">' + token + '</span>';
									...
									html+='<div class="zz" id="sp'+(i + 10000*k)+'an" style="background:#fff;width:200px;display:none;position:absolute;padding-top:0;" ></div>';
									/*var arr=[1,2,3,4];
									//alert(num3.length);
...
function save(obj){	
	...
	if(sign==true){
		html+='<a style="display:block;" class="spanClass" onclick="change(' + num + ',\''+content+'\')">'+content+'</a>';
	...
	$.post("translate.php",
           ...
           function(data) { 
			var data_arr = new Array();
				...
				for(var num2=0;num2<arr.length;num2++){
										//alert(num2)
					html+='<a style="display:block;" class="spanClass" onclick="change(' + num + ',\''+arr[num2]+'\')">'+arr[num2]+'</a>';
				}
				html+='<input type="txt" class="xx" value="'+txt+'" style="font-size:14px;width:140px;" id="input' + key + '"><input type="button" value="使用"   onclick="inputChange(' + num + ',input'+key+')" style="width:50px; margin-left:5px;font-size:14px;"></div>';
				document.getElementById(divstyle).style.border="1px solid #333";
				$("#"+divstyle).html(html);
